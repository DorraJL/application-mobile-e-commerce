...
class OrderDeliveredPage extends StatelessWidget {
  ...
  Widget build(BuildContext context)
  {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: Colors.white,
        title: TextFrave(text: 'Ordres Livrés'),
        centerTitle: true,
        elevation: 0,
        leadingWidth: 80,
        leading: InkWell(
          ...
          child: Row(
            ...
            children: [
              Icon(Icons.arrow_back_ios_new_rounded, size: 17, color: ColorsFrave.primaryColorFrave),
              TextFrave(text: 'Retour', fontSize: 17, color: ColorsFrave.primaryColorFrave )
            ],
          ),
        ),
      ),
      body: FutureBuilder<List<OrdersResponse>?>(
        future: deliveryController.getOrdersForDelivery('LIVRE'),
    ...
    return ( listOrdersDelivery.length != 0 ) 
      ...
      : Column(
        ...
        children: [
          ...
          TextFrave(text: 'Sans ordres livrés', color: ColorsFrave.primaryColorFrave, fontWeight: FontWeight.w500, fontSize: 21)
        ],
      );
  }
}